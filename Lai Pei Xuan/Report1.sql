...
DBMS_OUTPUT.PUT_LINE(RPAD('Day',18,' ')||':'|| TO_CHAR(SYSDATE,'DAY'));
...
FOR SUPPLIER_REC IN SUPPLIER_CURSOR LOOP
    ...
    DBMS_OUTPUT.PUT_LINE(RPAD('Purchase Transaction ID', 25, ' ') || RPAD('Product Code', 15, ' ') || RPAD('Product Name', 60, ' ') || RPAD('Quantity Purchase', 20, ' ') || RPAD('Buy Price (RM) ', 20, ' ') || RPAD('Total Amount (RM)', 20, ' '));
    DBMS_OUTPUT.PUT_LINE(RPAD('-', 160, '-'));
    ...
    FOR PURCHASE_REC IN PURCHASE_CURSOR(SUPPLIER_REC.SupplierName) LOOP
        ...
        DBMS_OUTPUT.PUT_LINE(RPAD(PURCHASE_REC.PurchaseTransID, 25, ' ') || RPAD(PURCHASE_REC.ProductCode, 15, ' ') || RPAD(PURCHASE_REC.ProductName, 60, ' ') || LPAD(PURCHASE_REC.QtyPurchase, 20, ' ') || RPAD(TO_CHAR(PURCHASE_REC.BuyPrice, '999999.99'), 20, ' ') || LPAD(TO_CHAR(v_total, '999999.99'), 20, ' '));
